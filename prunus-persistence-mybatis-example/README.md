...
즉, 필요한 audit field 정보만 정의하여 사용합니다.
```java
@AttributeOverrides({
        @AttributeOverride(name="createdBy", column=@Column(name="CRE_USER")),
        @AttributeOverride(name="createdDate", column=@Column(name="CRE_DATE")),
...
각각의 DB 에 대응하는 부분범위 SQL 으로 적용하여 동작하는 기능을 제공 합니다.
...
- `POST` Body
    ...
    ```
### Dialect
각 DB 에 대응하는 dialect 목록은 다음과 같습니다.
...
Dialect 는 spring bean 으로 등록되어 관리되며, 기본으로 제공되는 것 이외 Dialect 추가 시 다음과 같은 방법으로 가능합니다.
- Dialect class 구현
  com.github.miemiedev.mybatis.paginator.dialect.Dialect 클래스를 상속받아서   
  getLimitString 메서드를 각 DB SQL 에 특화된 사항에 따라 구현 합니다.
- Spring Bean 등록
  prunus.persistence.mybatis.pagination.interceptor.PagingDialect 객체의 파라미터로 생성된 Dialect class 를 전달하여 bean 으로 등록합니다.   
...
pagination 의 기능은 `spring-data-commons` 을 사용함으로서, spring-jpa 에서 사용하는 기능과 동일하도록 구현되어 있습니다.   
따라서, 해당 설정을 그대로 이용합니다.
...
spring:
  data:
    web:
      pageable:
        one-indexed-parameters: true # 페이지 번호를 1부터 시작할지 여부를 의미합니다. spring-jpa 해당 옵션은 기본으로 false 이지만, 통상의 사용법에 따른 1부터 시작을 위해 기본 값을 true 로 지정하였습니다.
...
하지만, page number 의 경우 해당 값이 항상 -1 값으로 반환 됩니다.   
...
클라이언트에서 전달 된 page number 값은 올바르게 전달되어 해당 범위의 data 도 정확하게 반환 하지만,   
`Pageable` 과 `Page` 객체 내부의 page number 관련 값은 여전히 항상 -1 이 반영되어 있습니다.   
따라서 반환 타입을 Page&lt;T&gt; 으로 선언하여 클라이언트로 전달 받아 사용할 경우   
page number 를 참조할 때 반드시 +1 처리를 해야 합니다.
